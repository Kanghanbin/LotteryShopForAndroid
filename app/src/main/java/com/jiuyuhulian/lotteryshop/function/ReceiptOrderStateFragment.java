...
public class ReceiptOrderStateFragment extends BaseFragment implements XRecyclerView.LoadingListener {
    ...
    public void getData(int page, final String type) {
        apiServices.mineOrder(shopId, 5, page).subscribeOn(Schedulers.io())
                ...
                .subscribe(new Subscriber<MineOrder>() {
                    ...
                    public void onNext(MineOrder mineOrder) {
                        handReceiptStateResult(mineOrder, type);
                    }
                });
    }

    private void handReceiptStateResult(MineOrder mineOrder, String type) {
        if (mineOrder.getCode() == 0) {
            if (type.equals(LOAD_MORE)) {
                ...
                adpter = new CommonAdapter<MineOrder.DataBean>(getActivity(), R.layout.receipt_order_adapter_item, dataBeans) {
                    @Override
                    protected void convert(final ViewHolder holder, final MineOrder.DataBean orderEntity, final int position) {
                        ...
                        holder.getView(R.id.btn_interflow).setOnClickListener(new View.OnClickListener() {
                            ...
                            public void onClick(View v) {
                                startActivity(new Intent(getActivity(), QueryInterflowActivity.class));
                            }
                        });
                        holder.getView(R.id.btn_confirm_receipt).setOnClickListener(new View.OnClickListener() {
                            ...
                            public void onClick(View v) {
                                new SweetAlertDialog(mContext, SweetAlertDialog.WARNING_TYPE)
                                        .setContentText("确认要收货吗？")
                                        .setConfirmClickListener(new SweetAlertDialog.OnSweetClickListener() {
                                            @Override
                                            public void onClick(SweetAlertDialog sweetAlertDialog) {
                                                receiptOrder(orderEntity,sweetAlertDialog);
                                            }
                                        })
    ...
    private void receiptOrder(MineOrder.DataBean orderEntity, final SweetAlertDialog dialog) {
        apiServices.confirmReceipt(orderEntity.getOrder_id()).subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
